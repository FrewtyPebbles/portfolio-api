...
class SymmetricAuthTokens(object):
	...
	def login(self, authID:str, accNotFoundMSG:str="Account not found", hashAuthID:bool = True, **kwargs:str) -> dict:
		"""
		 ...
		 - hashAuthID determines wether or not to hash the authID.  If this is a sensitive parameter, you should consider doing so.  It can be potentially helpful to leave this unhashed if you wish to use it to look up a user's account when SymmetricAuthTokens.checkLogin() is invoked.  This is true by default.
		"""
		...
		authIDHash = self.hashGen.hexdigest()
		self.cur.execute(SQLstatement, (authIDHash if hashAuthID else authID, Fkey, tokenStringToEncrypt,))
		authJson = {
			"authToken": encryptedStr.decode(),
			"authID": authIDHash if hashAuthID else authID,
			"error":"none"
		}
		#clean up old tokens
	...
	def initAccountSystem(self, requiredParameters:tuple | list = [], authParameters:tuple | list = [], parameterDefaults:dict = {}, **kwargs:str) -> bool:
		'''
		 ...
		 - parameterDefaults is a dictionary that includes default values for any non-required auth parameters.  Their values should be written with the same syntax style as SQL literals.
		...
		for key, value in kwargs.items():
			...
			if key in requiredParameters:
				...
				SQLStatement += f"""{key} {value} DEFAULT {parameterDefaults[key] if key in parameterDefaults.keys() else '"Not provided."'} NOT NULL, """
